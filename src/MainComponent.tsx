...
import { DEFAULT_VIEW, VIEW_DEFINITIONS, type ViewId } from './views';
...
const MainComponent = () => {
    ...
    const handleConnect = async () => {
        const connected = await connect({
            ...
            bootUrl: selectedConnection.bootUrl,
            passcode,
    ...
    const renderActiveView = () => {
        const view = VIEW_DEFINITIONS.find((item) => item.id === activeView);

        if (view?.requiresConnection && !client) {
            return renderConnectionRequired();
        }

        if (activeView === 'identifiers') {
            return client ? (
                <IdentifierTable client={client} />
            ) : (
...
const IdentifierTable = ({ client }: { client: SignifyClient }) => {
    ...
    Object.keys(currentIdentifier)[2],currentIdentifier[Object.keys(currentIdentifier)[2]]
            ...
            <pre>
                {JSON.stringify(
                    currentIdentifier[Object.keys(currentIdentifier)[2]],
                    null,
                    2
                )}
            </pre>
                        ...
                        typeof value === 'string' ? (
                            <Grid size={12} key={key}>
                                <Typography
                                    ...
                                    align="left"
                                >
                                    <strong>{tableObject[key].title}</strong>{' '}
                                    {value}
                                </Typography>
                            </Grid>
                        ) : null
